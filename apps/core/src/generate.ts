...
- The live preview ONLY shows the / route (app/page.tsx) and the user cannot change the URL. So the user's request must land on app/page.tsx (the home page) — e.g. "a features page" means build it INTO app/page.tsx, NOT a new app/features/page.tsx the user will never see. Only add a separate route when the app genuinely needs multiple pages, and then link to it from app/page.tsx so it's reachable.
